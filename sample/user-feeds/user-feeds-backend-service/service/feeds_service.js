...
module.exports = {
    getUserFeeds1:function(userId, pageNumber, callback) {
		...
		tweetApi.getUserTweets(parseInt(userId)).then(resp => {
			var docs = [];
			resp.data.data.forEach((d, index) => {
				if (index >= (pageNumber - 1) * 2 && index < 2 * pageNumber) {
					var tweetObj = {};
					tweetObj["_id"] = d._id;
					tweetObj["content"] = d.content;
					tweetObj["userName"] = d.userName;
					tweetObj["twitterHandle"] = d.twitterHandle;
			...
			if ((pageNumber + 1) * 2 < resp.data.data.length)
				result.hasMore = true;
			else
