...
module.exports = function (router) {
    ...
    router.get('/tweetById/:tweetId', function (req, res) {
        kafka.make_request(TOPICS.TWITTER_FEED_REQUEST_TOPIC, 'get-tweet-byId', req.params, req.body, function(err,doc){
            if (err){
                res.status(500).json({
                    message: 'Error getting tweet by tweet id',
                    error: err
                })
            }else{
                res.status(200).json(doc);
