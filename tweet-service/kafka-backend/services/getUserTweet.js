...
var userFollowingURL = "http://ec2-18-222-168-97.us-east-2.compute.amazonaws.com:4010/api/userFollowing/";
var mydb;
                ...
                Request.get(userFollowingURL + msg.userid, (error, res, body) => {
                    if(!error) {
                        ...
                        if (body.followingCount == 0) {
                            ...
                            tweet.find({by: {$in: following}}).sort({"date": -1}).toArray(function(err,result){
                                if (!err) {
