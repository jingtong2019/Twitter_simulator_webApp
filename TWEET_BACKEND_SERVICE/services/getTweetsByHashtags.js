...
function handle_request(msg, callback){
    ...
    if (!msg.hasOwnProperty('hashtag')) {
        ...
        tweet.find({hashtags: { $elemMatch: { $eq: msg.hashtag } }, tweet_type: { $ne: "COMMENT" } }).sort({"date": -1}).toArray(function(err,result){
            if (!err) {
                response.code = "200";
                response.value = "Successfully get all tweets of hashtag";
